...
class SampleSinuisoid:
    ...
    def __init__(self, x_range=(-5.0, 5.0), amplitude_range= (0.1, 5.0), phase_range=(0, np.pi), amp_params=(1,1), phase_params=(1,1)):
        super().__init__()
        ...
        self.support = D.Uniform(*self.x_range)
        self.amplitude = D.Beta(*self.amp_params)
        self.phase = D.Beta(*self.phase_params)
    ...
    def __str__(self) -> str:
        return f'Random Sinusoid. Amp: {self.cur_amplitude.item():2f}, '\
            + f'Phase: {self.cur_phase.item():2f}'


class SinusoidRegressor(nn.Module):
