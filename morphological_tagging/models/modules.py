...
class char2word(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_project = nn.Linear(
            in_features=(2 if bidirectional else 1) * h_dim,
    ...
    def forward(self, chars: torch.Tensor, char_lens: torch.Tensor):
        ...
        packed_c_embeds = pack_padded_sequence(c_embeds, char_lens, enforce_sorted=False)
...
class residual_lstm(nn.Module):
    ...
    def __init__(self, **lstm_kwargs) -> None:
        super().__init__()
    ...
    def forward(self, x):
        ...
        if isinstance(x, torch.nn.utils.rnn.PackedSequence):
            ...
            ht_out_, lens = pad_packed_sequence(ht_out)
            ht_out = pack_padded_sequence(x_ + ht_out_, lens, enforce_sorted=False)
        else:
            ht_out = x + ht_out
